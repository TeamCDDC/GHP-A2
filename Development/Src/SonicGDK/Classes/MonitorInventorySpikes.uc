...
//================================================================================
// Monitor Inventory Spikes > MonitorInventory > UTTimedPowerup > UTInventory >
//     > Inventory > Actor
//
// The parent class represents the item object that is given to players.
// This subclass grants the "spikes" orb.
//================================================================================
class MonitorInventorySpikes extends MonitorInventory;


/**The related music track.*/ var SoundCue SpikesSoundCue;
...
simulated event Tick(float DeltaTime)
{
    ...
    {
        if (Owner.IsInState('OrbSpikes'))
        {
            if (Owner.Physics == PHYS_Spider)
        ...
        else
        {
            ...
            if (Pawn(Owner).Controller != none)
                SGDKPlayerController(Pawn(Owner).Controller).GetMusicManager().StopMusicTrack(SpikesSoundCue,1.0);
...
{
    P.GotoState('OrbSpikes');

    if (P.IsInState('OrbSpikes'))
    {
        Energy = 100.0;

        if (P.Controller != none)
            SGDKPlayerController(P.Controller).GetMusicManager().StartMusicTrack(SpikesSoundCue,0.0);
    }
}
...
function StopEffect(SGDKPlayerPawn P)
{
    ...
    if (P.Controller != none)
        SGDKPlayerController(P.Controller).GetMusicManager().StopMusicTrack(SpikesSoundCue,1.0);
}
...
defaultproperties
{
    ...
    EnergyHudCoords=(U=944,V=408,UL=70,VL=70)
    PickupSound=SoundCue'SonicGDKPackSounds.SpikesPickupSoundCue'
    TimeRemaining=60.0

    SpikesSoundCue=SoundCue'SonicGDKPackMusic.OrbSpikesSoundCue'
}
